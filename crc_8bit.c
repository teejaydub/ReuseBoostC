/*
	CRC-8 for Dallas iButton products
	
	
	From Maxim/Dallas AP Note 27
	
	"Understanding and Using Cyclic Redundancy Checks with 
	Dallas Semiconductor iButton Products"
	
	The Ap note describes the CRC-8 algorithm used in the 
	iButton products. Their implementation involves a 256 byte
	CRC table. This algorithm is implemented here. In addition
	two other algorithms are shown. One uses nibble arrays and
	the other uses boolean arithmetic.
	
	
	18JAN03 - T. Scott Dattalo
	
	Modified by Timothy Weber.
*/

#include <system.h>

#define IN_CRC_8BIT

#include "crc_8bit.h"


// Choose your favorite implementation by defining (only) one of these macros.
#if !defined(CRC8_IMP_TABLE) && !defined(CRC8_IMP_NIBBLES) && !defined(CRC8_IMP_BITS)
 #define CRC8_IMP_BITS
#endif


#ifdef CRC8_IMP_TABLE

// crc array from the Maxim ApNote
rom char* crc_array = {
	, 
};

unsigned char crc8(unsigned char data)
{
	unsigned char i = (data ^ crc);
	
	crc = crc_array[i];
	return crc;
}

#endif

#ifdef CRC8_IMP_NIBBLES

// CRC arrays for the nibble-wise routine.
rom char* r1 = {
	0x00, 0x5e, 0xbc, 0xe2, 0x61, 0x3f, 0xdd, 0x83, 
	0xc2, 0x9c, 0x7e, 0x20, 0xa3, 0xfd, 0x1f, 0x41, 
};

rom char* r2 = {
	0x00, 0x9d, 0x23, 0xbe, 0x46, 0xdb, 0x65, 0xf8,
	0x8c, 0x11, 0xaf, 0x32, 0xca, 0x57, 0xe9, 0x74
};

unsigned char crc8(unsigned char data)
{
	unsigned char i = (data ^ crc);
	
	crc = r1[i & 0xf] ^ r2[i >> 4];
	return crc;
}
#endif

#ifdef CRC8_IMP_BITS
unsigned char crc8(unsigned char data)
{
	unsigned char i = (data ^ crc);
	
	crc = 0;
	
	if(i & 1)
		crc ^= 0x5e;
	if(i & 2)
		crc ^= 0xbc;
	if(i & 4)
		crc ^= 0x61;
	if(i & 8)
		crc ^= 0xc2;
	if(i & 0x10)
		crc ^= 0x9d;
	if(i & 0x20)
		crc ^= 0x23;
	if(i & 0x40)
		crc ^= 0x46;
	if(i & 0x80)
		crc ^= 0x8c;
	
	return crc;
}
#endif